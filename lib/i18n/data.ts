import type { Locale } from '@/contexts/LocaleContext'

// Bozza di traduzione automatica delle stringhe UI principali (navbar). Da rivedere
// con un madrelingua prima di considerarla definitiva.
export const dictionary = {
  it: {
    nav_home: 'Home',
    nav_products: 'Prodotti & Shop',
    nav_quality: 'Qualità 08',
    nav_blog: 'Blog',
    nav_careers: 'Lavora con noi',
    nav_contact: 'Contatti',
    nav_cart: 'Carrello',
    nav_login: 'Accedi',
    nav_account: 'Il mio account',
    nav_admin: 'Pannello admin',
    nav_logout: 'Esci',
    nav_register: 'Registrati',
    nav_menu: 'Menu',
    lang_selector: 'Selettore lingua',
    lang_coming_soon: 'Prossimamente',

    // Aggiungi al carrello / disponibilità
    cart_out_of_stock: 'Esaurito',
    cart_decrease_qty: 'Diminuisci quantità',
    cart_increase_qty: 'Aumenta quantità',
    cart_added: 'Aggiunto al carrello',
    cart_add: 'Aggiungi al carrello',

    // Cassetto carrello
    cart_title: 'Carrello',
    cart_empty: 'Il carrello è vuoto',
    cart_continue_shopping: 'Continua gli acquisti',
    cart_free_shipping_earned: 'Hai diritto alla spedizione gratuita!',
    cart_free_shipping_missing: 'Aggiungi {amount} per la spedizione gratuita',
    cart_coupon_placeholder: 'Codice sconto',
    cart_coupon_apply: 'Applica',
    cart_coupon_applied: 'Codice {code} applicato',
    cart_coupon_remove: 'Rimuovi',
    cart_subtotal: 'Subtotale',
    cart_discount: 'Sconto',
    cart_shipping: 'Spedizione',
    cart_shipping_free: 'Gratuita',
    cart_total: 'Totale',
    cart_checkout: "Procedi all'acquisto",
    cart_checkout_note: 'Supplemento estero e contrassegno calcolati al checkout',

    // Footer
    footer_company: 'Azienda',
    footer_legal: 'Legale',
    footer_couriers: 'Corrieri',
    footer_rights: 'Tutti i diritti riservati',
    footer_made_in_italy: 'Made in Italy',
    footer_returns_shipping: 'Resi e spedizioni',
    footer_terms: 'Termini e condizioni',
    footer_privacy: 'Privacy policy',
    footer_cookie_policy: 'Cookie policy',
    footer_legal_notices: 'Note legali',

    // Home
    home_hero_title: "L'**Eccellenza**\ncome Standard.",
    home_hero_body:
      '08 Natural Technology nasce da un sogno: creare qualcosa di cui poter essere veramente orgogliosi. Non volevamo semplicemente realizzare degli integratori, ma dare vita a prodotti sviluppati con passione, attenzione e rispetto per le persone che ogni giorno ripongono la loro fiducia in noi.',
    home_hero_cta: 'Scopri le formule',
    home_hero_alt: '08 Natural Technology',
    home_lab_alt: 'Laboratorio 08 Natural Technology',

    home_feature_1_title: 'Eccellenza italiana',
    home_feature_1_body: 'Identità italiana, cura del dettaglio e standard elevati in ogni scelta.',
    home_feature_2_title: 'Ingredienti di alta qualità',
    home_feature_2_body: 'Materie prime selezionate con attenzione e formule coerenti.',
    home_feature_3_title: 'Ricerca e innovazione',
    home_feature_3_body: 'Soluzioni nutrizionali moderne, ad alta biodisponibilità.',
    home_feature_4_title: 'Benessere e risultati concreti',
    home_feature_4_body: 'Soluzioni concrete, pensate per esigenze mirate.',

    home_about_eyebrow: 'Chi siamo',
    home_about_script: 'Lettera del Fondatore',
    home_about_title: 'Un sogno diventato\n**promessa quotidiana.**',
    home_about_quote:
      'Per noi il benessere non è una semplice parola: è energia, equilibrio, serenità e qualità della vita.',
    home_about_p1:
      "**08 Natural Technology** nasce dal desiderio di creare integratori alimentari curati nei dettagli, con formule comprensibili, ingredienti selezionati e un'identità italiana riconoscibile.",
    home_about_p2:
      'Non volevamo realizzare semplicemente prodotti, ma un marchio con formule capaci di trasmettere fiducia, attenzione e rispetto verso chi ogni giorno sceglie di prendersi cura di sé.',
    home_about_signature: 'Vito Tatulli',

    home_formulas_eyebrow: 'Le nostre formule',
    home_formulas_title: 'Prodotti pensati\nper **esigenze reali.**',
    home_formulas_body:
      "Dai prodotti per microcircolo e gambe leggere, fino al supporto vitaminico, alla bellezza di capelli, pelle e unghie e all'equilibrio femminile in menopausa: 08 Natural Technology propone soluzioni nutrizionali pensate per esigenze concrete e quotidiane.",
    home_formulas_cta: 'Scopri i prodotti',

    home_blog_eyebrow: 'Blog 08',
    home_blog_title: 'Approfondimenti e benessere quotidiano',
    home_blog_body:
      'Scopri consigli, ingredienti e articoli utili per orientarti meglio tra formule, esigenze e scelte quotidiane.',
    home_reading_time: 'min di lettura',

    // Contatti
    contact_eyebrow: 'Parliamo',
    contact_title: 'Contatti.',
    contact_where_to_find_us: 'Dove trovarci',
    contact_company_title: 'VIPHARMA di\n**Tatulli Vito & Co.**',
    contact_row_address_label: 'Indirizzo',
    contact_row_address_value: 'Via Don Luigi Sturzo 44/46/48 - Bitonto (BA) 70032',
    contact_row_phone_label: 'Telefono',
    contact_row_whatsapp_label: 'WhatsApp',
    contact_row_email_label: 'Email',
    contact_row_social_label: 'Social',
    contact_row_social_value: 'Seguici per aggiornamenti e contenuti.',
    contact_write_us: 'Scrivici',
    contact_form_title: 'Mandaci un\n**messaggio.**',
    contact_field_name: 'Nome e cognome',
    contact_field_email: 'Email',
    contact_field_subject: 'Oggetto',
    contact_subject_products: 'Informazioni sui prodotti',
    contact_subject_collab: 'Collaborazioni',
    contact_subject_distribution: 'Distribuzione',
    contact_subject_other: 'Altro',
    contact_field_message: 'Messaggio',
    contact_send: 'Invia messaggio',
    contact_field_error: 'Compila correttamente nome, email e messaggio.',

    // B2B
    b2b_eyebrow: 'B2B',
    b2b_hero_title: 'Diventa rivenditore\n**08 Natural Technology.**',
    b2b_hero_p1:
      'Pagina dedicata a farmacie, parafarmacie, erboristerie e operatori del settore. Qui il sito raccoglie contatti qualificati e richieste catalogo da partner commerciali interessati alla linea 08 Natural Technology.',
    b2b_hero_p2:
      'Siamo presenti in farmacia, parafarmacia ed erboristeria. Se sei un operatore del settore e vuoi portare 08 nel tuo punto vendita, contattaci per ricevere il catalogo commerciale e le condizioni di fornitura.',
    b2b_hero_cta: 'Richiedi informazioni',
    b2b_offer_title: 'Cosa offriamo\nal **rivenditore.**',
    b2b_offer_1: 'Catalogo prodotti',
    b2b_offer_2: 'Schede tecniche PDF',
    b2b_offer_3: 'Listino commerciale',
    b2b_offer_4: 'Materiale vetrina',
    b2b_offer_5: 'Contenuti social',
    b2b_offer_6: 'Supporto formazione',
    b2b_offer_7: 'Assistenza commerciale',
    b2b_offer_8: 'Materiali promozionali',
    b2b_zone_label: 'Zona di distribuzione',
    b2b_why_eyebrow: 'Perché 08',
    b2b_why_title: 'Un marchio che costruisce fiducia\nnel **punto vendita.**',
    b2b_trust_1_label: 'Prodotti notificati',
    b2b_trust_1_body:
      'Integratori alimentari notificati al Ministero della Salute della Repubblica Italiana dove indicato.',
    b2b_trust_2_label: 'Made in Italy',
    b2b_trust_2_body:
      'Progetto nato a Bitonto, in Puglia. Identità italiana chiara e riconoscibile.',
    b2b_trust_3_label: 'Formule curate',
    b2b_trust_3_body:
      'Ingredienti selezionati, dosaggi dichiarati, comunicazione responsabile e trasparente.',

    // Checkout
    checkout_empty: 'Il carrello è vuoto.',
    checkout_go_to_products: 'Vai ai prodotti',
    checkout_data_shipping: 'Dati e spedizione',
    checkout_email: 'Email',
    checkout_email_placeholder: 'cliente@esempio.test',
    checkout_email_note: "Riceverai la conferma dell'ordine a questo indirizzo.",
    checkout_already_account: 'Hai già un account?',
    checkout_create_account: 'Crea un account con questi dati',
    checkout_password: 'Password',
    checkout_password_placeholder: 'Crea una password sicura',
    checkout_password_note:
      'Potrai accedere con questa email e password per consultare i tuoi ordini.',
    checkout_first_name: 'Nome',
    checkout_first_name_placeholder: 'Nome intestatario',
    checkout_last_name: 'Cognome',
    checkout_last_name_placeholder: 'Cognome intestatario',
    checkout_fiscal_doc: 'Documento fiscale',
    checkout_doc_none: 'Nessun documento',
    checkout_doc_receipt: 'Scontrino',
    checkout_doc_invoice: 'Fattura',
    checkout_fiscal_code_optional: 'Codice fiscale (opzionale)',
    checkout_fiscal_code_placeholder: 'Codice fiscale se richiesto',
    checkout_company: 'Ragione sociale',
    checkout_company_placeholder: 'Nome azienda',
    checkout_vat: 'C. FISCALE o Partita IVA',
    checkout_vat_placeholder: 'Codice fiscale o partita IVA',
    checkout_sdi_code: 'Codice SDI',
    checkout_pec: 'PEC',
    checkout_sdi_or_pec_note: 'Inserisci almeno Codice SDI oppure PEC.',
    checkout_address: 'Indirizzo',
    checkout_address_placeholder: 'Via e numero civico',
    checkout_city: 'Città',
    checkout_city_placeholder: 'Città di consegna',
    checkout_postal_code: 'CAP',
    checkout_postal_code_placeholder: 'Codice postale',
    checkout_province: 'Provincia',
    checkout_province_placeholder: 'Sigla provincia',
    checkout_phone_optional: 'Telefono (opzionale)',
    checkout_phone: 'Telefono',
    checkout_phone_prefix_label: 'Prefisso telefonico',
    checkout_phone_placeholder: 'Numero di telefono',
    checkout_shipping_country: 'Paese di spedizione',
    checkout_foreign_surcharge_note: 'Supplemento spedizione estera: +{amount}',
    checkout_billing_different: "L'indirizzo di fatturazione è diverso da quello di spedizione",
    checkout_billing_address_title: 'Indirizzo di fatturazione',
    checkout_billing_first_name_placeholder: 'Nome fatturazione',
    checkout_billing_last_name_placeholder: 'Cognome fatturazione',
    checkout_company_optional: 'Azienda (opzionale)',
    checkout_vat_optional: 'C. FISCALE o Partita IVA (opzionale)',
    checkout_billing_city_placeholder: 'Città fatturazione',
    checkout_province_short: 'Prov.',
    checkout_country: 'Paese',
    checkout_save_next_time: 'Salva i dati per il prossimo ordine',
    checkout_shipping_notes: 'Note per la spedizione (opzionale)',
    checkout_shipping_notes_placeholder: 'Es. citofono, piano, fascia oraria preferita',
    checkout_delivery_method: 'Modalità di consegna',
    checkout_delivery_home: 'Consegna a domicilio',
    checkout_delivery_home_desc: "Il corriere consegna all'indirizzo indicato",
    checkout_delivery_pickup: 'Ritiro in punto di ritiro',
    checkout_delivery_pickup_desc: 'Scegli un punto BRT o Poste Italiane vicino a te',
    checkout_islands_poste_only: 'Per le isole è disponibile solo **Poste Italiane** come punto di ritiro.',
    checkout_carrier: 'Corriere',
    checkout_carrier_brt: 'BRT Fermopoint',
    checkout_carrier_poste: 'Poste Italiane',
    checkout_pickup_search_zip: 'CAP',
    checkout_pickup_search_zip_placeholder: 'es. 70032',
    checkout_pickup_search_button: 'Cerca',
    checkout_pickup_none_found: 'Nessun punto di ritiro trovato',
    checkout_pickup_zip_error: 'Inserisci un CAP valido (5 cifre)',
    checkout_pickup_network_error: 'Errore di rete durante la ricerca',
    checkout_brt_use_location: 'Usa la mia posizione',
    checkout_brt_locating: 'Ricerca in corso…',
    checkout_brt_distance: 'a {distance}',
    checkout_brt_type_locker: 'Locker',
    checkout_brt_type_shop: 'Punto BRT',
    checkout_brt_geolocation_unavailable: 'Geolocalizzazione non disponibile su questo dispositivo',
    checkout_brt_geolocation_denied: 'Permesso di geolocalizzazione negato',
    checkout_brt_invalid_coordinates: 'Coordinate non valide',
    checkout_brt_service_unavailable: 'Servizio BRT non disponibile al momento',
    checkout_brt_cod_disabled: 'Il contrassegno non è disponibile con il ritiro in punto BRT.',
    checkout_poste_search_label: 'Cerca punto di ritiro per CAP',
    checkout_poste_zip_placeholder: 'es. 00164',
    checkout_poste_hours_weekday: 'Lun-Ven',
    checkout_poste_hours_saturday: 'Sab',
    checkout_poste_config_missing: "La selezione automatica richiede l'attivazione delle credenziali Poste Italiane.",
    checkout_poste_service_unavailable: 'Servizio Poste Italiane non disponibile al momento',
    checkout_brt_phone_required: 'Il telefono è obbligatorio per il ritiro in punto di ritiro',
    checkout_discount_code: 'Codice sconto',
    checkout_discount_code_placeholder: 'Codice promozionale',
    checkout_apply: 'Applica',
    checkout_remove: 'Rimuovi',
    checkout_code_discount_applied: 'Codice {code} — sconto {amount}',
    checkout_payment_method: 'Metodo di pagamento',
    checkout_pay_stripe_label: 'Carta / PayPal / Google Pay / Apple Pay',
    checkout_pay_stripe_desc: 'Pagamento online sicuro con Stripe',
    checkout_pay_bank_label: 'Bonifico bancario',
    checkout_pay_bank_desc: 'Riceverai IBAN e causale dopo la conferma',
    checkout_pay_cod_label: 'Contrassegno',
    checkout_pay_cod_desc: 'Paghi in contanti al corriere (+{amount} supplemento)',
    checkout_processing: 'Elaborazione…',
    checkout_proceed_payment: 'Procedi al pagamento →',
    checkout_confirm_order: 'Conferma ordine →',
    checkout_error_email: 'Inserisci un indirizzo email valido per continuare.',
    checkout_error_password: 'La password deve essere di almeno 8 caratteri.',
    checkout_edit_address: 'Modifica indirizzo',
    checkout_payment_title: 'Pagamento',
    checkout_order_summary: 'Riepilogo ordine',
    checkout_discount_with_code: 'Sconto ({code})',
    checkout_free: 'Gratuita',

    // Newsletter
    newsletter_kicker: 'Newsletter 08',
    newsletter_title: 'Extra sconto 5%',
    newsletter_body:
      'Iscriviti per ricevere novità, contenuti e formule 08 Natural Technology.',
    newsletter_button: 'Iscriviti',
    newsletter_email_label: 'Email',
    newsletter_email_placeholder: 'nome@email.it',
    newsletter_sending: 'Invio...',
    newsletter_consent_before: 'Ho letto l',
    newsletter_consent_link: 'informativa privacy',
    newsletter_consent_after:
      'e acconsento a ricevere comunicazioni commerciali. Puoi cancellarti in ogni momento.',
    newsletter_success:
      'Ti abbiamo inviato una email: conferma il tuo indirizzo per ricevere il codice.',
    newsletter_error: 'Non siamo riusciti a completare l iscrizione.',
    newsletter_optin_register:
      'Voglio ricevere la newsletter con novità e offerte. Puoi cancellarti in ogni momento.',
    newsletter_account_title: 'Newsletter',
    newsletter_account_active: 'Sei iscritto alla newsletter.',
    newsletter_account_inactive: 'Non sei iscritto alla newsletter.',
    newsletter_account_subscribe: 'Iscrivimi',
    newsletter_account_unsubscribe: 'Cancella iscrizione',
    checkout_foreign_surcharge: 'Supplemento estero',
    checkout_cod_surcharge: 'Supplemento contrassegno',
    checkout_trust_stripe: 'Pagamento sicuro con Stripe',
    checkout_trust_bank: 'Riceverai le coordinate bancarie via email',
    checkout_trust_cod: 'Paghi in contanti alla consegna',
    checkout_payment_error: 'Errore nel pagamento',
    checkout_confirm_and_pay: 'Conferma e paga',

    // Checkout: pagina di successo
    success_order_received: 'Ordine ricevuto',
    success_thank_you: 'Grazie per il tuo acquisto.',
    success_bank_instructions_title: 'Istruzioni bonifico bancario',
    success_bank_instructions_body:
      "Effettua il bonifico entro **5 giorni lavorativi**. L'ordine sarà processato alla ricezione del pagamento.",
    success_bank_holder: 'Intestatario',
    success_bank_iban: 'IBAN',
    success_bank_reason: 'Causale',
    success_bank_reason_fallback: 'Numero ordine (vedi email)',
    success_cod_title: 'Pagamento alla consegna',
    success_cod_body:
      "Il pagamento avviene direttamente al corriere al momento della consegna. Tieni pronto l'importo esatto in contanti. Il supplemento contrassegno di **€5,00** è già incluso nel totale.",
    success_stripe_body:
      'Il tuo ordine è stato ricevuto e confermato. Riceverai una email con i dettagli della spedizione non appena il pacco sarà in partenza.',
    success_confirmation_email_note:
      'Riceverai una email di conferma con il riepilogo del tuo ordine.',
    success_view_orders: 'Vedi i miei ordini',
    success_continue_shopping: 'Continua gli acquisti',

    // Pagina prodotti (listing)
    products_hero_title: 'I Nostri\n**Prodotti.**',
    products_hero_body: 'Formule pensate per esigenze reali, con ingredienti selezionati.',
    products_slide_kicker_suffix: 'Formula mirata',
    products_capsules: '{n} capsule vegetali',
    products_days: '{n} giorni',
    products_discover: 'Scopri',
    products_disclaimer:
      'Gli integratori alimentari non sostituiscono una dieta varia ed equilibrata e uno stile di vita sano. Tenere fuori dalla portata dei bambini. Tutti i prodotti sono notificati al Ministero della Salute della Repubblica Italiana dove indicato.',

    // Pagina prodotto (dettaglio) - avviso normativo
    product_regulatory_notice:
      'Integratore alimentare notificato al Ministero della Salute della Repubblica Italiana{ms}. Non superare la dose giornaliera consigliata. Gli integratori alimentari non sostituiscono una dieta varia ed equilibrata e uno stile di vita sano. Tenere fuori dalla portata dei bambini.',

    // Lavora con noi
    work_hero_title: 'Lavora **con noi.**',
    work_hero_body:
      'Cerchiamo collaborazioni costruite su competenza, fiducia e attenzione reale alla qualità del progetto.',
    work_eyebrow: 'Crescere con metodo',
    work_title: 'Un progetto giovane,\ncon **standard chiari.**',
    work_intro:
      '08 Natural Technology valuta collaborazioni con persone e realtà che condividono un approccio serio, chiaro e orientato al benessere quotidiano. Ogni rapporto nasce dal rispetto per il prodotto, per il cliente e per il modo in cui il brand viene rappresentato.',
    work_area_1_title: 'Area commerciale',
    work_area_1_body:
      'Collaborazioni con figure capaci di raccontare il valore del prodotto con precisione, metodo e attenzione al cliente.',
    work_area_2_title: 'Consulenza e divulgazione',
    work_area_2_body:
      'Professionisti interessati a contribuire a un linguaggio chiaro, responsabile e coerente con la qualità 08.',
    work_area_3_title: 'Partner territoriali',
    work_area_3_body:
      'Relazioni con realtà locali, farmacie, parafarmacie, erboristerie e operatori del benessere.',
    work_cta_title: 'Raccontaci il tuo profilo.',
    work_cta_body:
      'Se pensi di poter contribuire alla crescita di 08 Natural Technology, inviaci una presentazione sintetica: esperienze, area di interesse e modalità di collaborazione proposta.',
    work_cta_button: 'Contattaci',

    // Blog
    blog_eyebrow: 'Scienza, nutrizione, benessere',
    blog_script: 'Approfondimenti',
    blog_title_1: 'Approfondimenti',
    blog_title_2: 'Blog.',
    blog_search_eyebrow: 'Ricerca articoli',
    blog_search_title: 'Trova il tema\nche **ti interessa.**',
    blog_search_field_label: 'Cerca nel blog',
    blog_search_placeholder: 'Es. diosmina, menopausa, microcircolo...',
    blog_section_eyebrow: 'Blog 08',
    blog_results_for: 'Risultati per "{query}"',
    blog_results_suggested: 'Articoli suggeriti in base al tema cercato.',
    blog_results_none: 'Nessun articolo trovato per questa ricerca.',
    blog_section_dossier_title: 'Dossier prodotto',
    blog_section_dossier_body: 'I post completi dedicati alle formule 08 Natural Technology.',
    blog_section_principles_title: 'Principi attivi, ingredienti e funzioni',
    blog_section_principles_body:
      'Post separati dedicati ai singoli attivi: cosa sono, quale ingrediente viene usato e quale funzione svolgono nella formula.',
    blog_section_quality_title: 'Qualità ingredienti: confronti utili',
    blog_section_quality_body:
      'Post separati che spiegano la differenza tra un ingrediente tecnico e la rispettiva alternativa generica.',
    blog_section_other_title: 'Approfondimenti già presenti',
    blog_section_other_body: "Articoli mantenuti nell'archivio blog.",
    blog_read_article: 'Leggi articolo',
    blog_reading_time: 'min di lettura',

    // Account
    account_title: 'Il mio account — 08 Natural Technology',
    account_eyebrow: 'Area personale',
    account_default_title: 'Il mio account',
    account_b2b_badge: 'Rivenditore B2B',
    account_stat_orders: 'Ordini effettuati',
    account_stat_total_spent: 'Spesa totale',
    account_my_orders: 'I miei ordini',
    account_no_orders_title: 'Nessun ordine ancora.',
    account_no_orders_body: 'Esplora il catalogo e trova il prodotto adatto a te.',
    account_discover_products: 'Scopri i prodotti',
    account_item_singular: 'articolo',
    account_item_plural: 'articoli',
    account_details: 'Dettagli',
    account_link_profile: 'Il mio profilo',
    account_link_catalog: 'Catalogo',
    account_link_support: 'Assistenza',
    order_status_pending: 'In attesa',
    order_status_paid: 'Pagato',
    order_status_shipped: 'Spedito',
    order_status_delivered: 'Consegnato',
    order_status_cancelled: 'Annullato',

    // Profilo
    profile_title: 'Il mio profilo — 08 Natural Technology',
    profile_back_to_account: 'Il mio account',
    profile_heading: 'Il mio profilo',
    profile_saved: 'Modifiche salvate',
    profile_personal_data: 'Dati personali',
    profile_full_name: 'Nome e cognome',
    profile_phone: 'Telefono',
    profile_fiscal_code: 'Codice fiscale',
    profile_email: 'Email',
    profile_company_data: 'Dati azienda',
    profile_company_data_hint: '(per fatturazione e acquisti B2B)',
    profile_company_name: 'Ragione sociale',
    profile_vat_number: 'Partita IVA',
    profile_sdi_code: 'Codice SDI',
    profile_pec: 'PEC',
    profile_save_changes: 'Salva modifiche',
    profile_shipping_addresses: 'Indirizzi di spedizione',
    profile_new_address: '+ Nuovo',
    profile_no_saved_address: 'Nessun indirizzo salvato',
    profile_add_address: 'Aggiungi indirizzo',
    profile_default_badge: 'Predefinito',
    profile_edit: 'Modifica',
    profile_set_default: 'Imposta come predefinito',
    profile_delete: 'Elimina',

    // Indirizzi (nuovo / modifica)
    address_new_heading: 'Nuovo indirizzo',
    address_edit_heading: 'Modifica indirizzo',
    address_form_label: 'Etichetta (opzionale)',
    address_form_label_placeholder: 'es. Casa, Ufficio, Magazzino',
    address_form_default: 'Predefinito',
    address_form_recipient: 'Intestatario',
    address_form_first_name: 'Nome',
    address_form_last_name: 'Cognome',
    address_form_company: 'Ragione sociale (aziende)',
    address_form_vat_number: 'Partita IVA',
    address_form_fiscal_code: 'Codice fiscale',
    address_form_address_section: 'Indirizzo',
    address_form_street: 'Via / Piazza',
    address_form_street_placeholder: 'es. Via Roma 1',
    address_form_postal_code: 'CAP',
    address_form_city: 'Città',
    address_form_province: 'Prov.',
    address_form_phone: 'Telefono',
    address_form_phone_placeholder: '+39 000 000 0000',
    address_form_country: 'Paese',
    address_form_save_new: 'Salva indirizzo',
    address_form_save_changes: 'Salva modifiche',
    address_form_delete: 'Elimina indirizzo',

    // Dettaglio ordine
    order_detail_back: 'I miei ordini',
    order_detail_label: 'Ordine',
    order_detail_items_ordered: 'Prodotti ordinati',
    order_detail_qty_each: 'Qtà {qty} · €{price} cad.',
    order_detail_amount_summary: 'Riepilogo importi',
    order_detail_subtotal: 'Subtotale',
    order_detail_discount: 'Sconto',
    order_detail_total: 'Totale',
    order_detail_shipping_address: 'Indirizzo di spedizione',
    order_detail_continue_shopping: 'Continua gli acquisti',
    order_detail_support: 'Assistenza',

    // Login
    login_eyebrow: 'Area clienti',
    login_title: 'Bentornato.',
    login_body: 'Accedi per vedere i tuoi ordini e gestire il profilo.',
    login_no_account: 'Non hai un account?',
    login_register_link: 'Registrati',
    login_email: 'Email',
    login_password: 'Password',
    login_forgot_password: 'Password dimenticata?',
    login_error_unverified:
      'Devi prima confermare la tua email. Controlla la tua casella di posta.',
    login_error_credentials: 'Email o password non corretti.',
    login_resend_sent: 'Email inviata di nuovo',
    login_resend_sending: 'Invio in corso…',
    login_resend_verification: "Invia di nuovo l'email di conferma",
    login_submit: 'Accedi',
    login_submitting: 'Accesso in corso…',
    login_or: 'oppure',
    login_google: 'Continua con Google',
    login_verify_success: 'Email confermata! Ora puoi accedere.',
    login_verify_expired:
      "Il link di conferma è scaduto. Richiedine uno nuovo qui sotto dopo aver effettuato l'accesso.",
    login_verify_invalid: 'Link di conferma non valido.',
    login_reset_success: 'Password aggiornata! Accedi con la nuova password.',

    // Registrati
    register_eyebrow: 'Nuovo account',
    register_title: 'Benvenuto.',
    register_body: 'Crea il tuo account per accedere agli ordini e alle promozioni riservate.',
    register_already_account: 'Hai già un account?',
    register_login_link: 'Accedi',
    register_name: 'Nome',
    register_name_placeholder: 'Mario Rossi',
    register_email_placeholder: 'mario@esempio.it',
    register_password: 'Password',
    register_password_placeholder: 'Minimo 8 caratteri',
    register_confirm_password: 'Conferma password',
    register_confirm_password_placeholder: 'Ripeti la password',
    register_password_mismatch: 'Le password non coincidono',
    register_generic_error: 'Errore durante la registrazione',
    register_network_error: 'Errore di rete. Riprova.',
    register_submit: 'Crea account',
    register_submitting: 'Creazione account…',
    register_terms_prefix: 'Registrandoti accetti i nostri',
    register_terms_link: 'Termini e condizioni di vendita',
    register_and_the: 'e la',
    register_privacy_link: 'Privacy Policy',
    register_check_email_title: 'Controlla la tua email',
    register_check_email_body:
      "Ti abbiamo inviato un link a **{email}** per confermare il tuo indirizzo. Clicca sul link entro 24 ore per attivare l'account.",
    register_email_failed_title: 'Account creato, email non inviata',
    register_email_failed_body:
      'Il tuo account **{email}** è stato creato, ma non siamo riusciti a inviare l\'email di conferma. Vai alla pagina di accesso e usa "Invia di nuovo l\'email di verifica".',

    // Password dimenticata / reimposta
    forgot_eyebrow: 'Recupero account',
    forgot_title: 'Password dimenticata?',
    forgot_body: 'Inserisci la tua email e ti invieremo un link per reimpostarla.',
    forgot_back_to_login: 'Torna al login',
    forgot_check_email_title: 'Controlla la tua email',
    forgot_check_email_body:
      'Se esiste un account associato a **{email}**, ti abbiamo inviato un link per reimpostare la password. Il link scade tra 1 ora.',
    forgot_submit: 'Invia link di reimpostazione',
    forgot_submitting: 'Invio in corso…',
    reset_title: 'Scegli una nuova password.',
    reset_body: 'Deve contenere almeno 8 caratteri.',
    reset_invalid_link: 'Link non valido.',
    reset_request_new_link: 'Richiedi un nuovo link →',
    reset_new_password: 'Nuova password',
    reset_confirm_new_password: 'Conferma nuova password',
    reset_password_mismatch: 'Le password non coincidono',
    reset_generic_error: 'Errore durante il reset della password',
    reset_submit: 'Reimposta password',
    reset_submitting: 'Salvataggio…',

    // Pagine legali
    legal_index: 'Indice',
    legal_last_updated: 'Ultimo aggiornamento',

    // Metodo / Qualità 08
    method_eyebrow: 'Qualità 08',
    method_hero_title: "L'**Eccellenza**\ncome Standard.",
    method_hero_body:
      'Non volevamo semplicemente realizzare degli integratori, ma dare vita a prodotti sviluppati con passione, attenzione e rispetto per le persone che ogni giorno ripongono la loro fiducia in noi.',
    method_hero_cta: 'Scopri le formule',
    method_promise_eyebrow: 'La nostra promessa',
    method_promise_title: 'Qualità, innovazione\ne **fiducia.**',
    method_promise_body:
      'Dietro ogni formula ci sono ricerca, impegno e una scelta accurata degli ingredienti, perché crediamo che la qualità non sia un dettaglio, ma un valore fondamentale.',
    method_panel1_title: 'Più di un marchio.',
    method_panel1_p1:
      'Per noi il benessere non è una semplice parola: è energia, equilibrio, serenità e qualità della vita. È qualcosa di prezioso che merita attenzione e cura.',
    method_panel1_p2:
      'Per questo lavoriamo ogni giorno con lo stesso obiettivo: offrire prodotti che uniscano qualità, innovazione e fiducia, mettendo sempre la persona al centro di ogni scelta.',
    method_panel1_li1: 'Formule sviluppate con attenzione.',
    method_panel1_li2: 'Ingredienti selezionati con cura.',
    method_panel1_li3: 'Comunicazione chiara e responsabile.',
    method_panel1_li4: 'Rispetto verso chi sceglie il nostro brand.',
    method_panel2_title: 'La promessa 08.',
    method_panel2_p1: 'La qualità, per noi, non è solo un obiettivo: è un metodo.',
    method_panel2_p2:
      'Ogni formula 08 Natural Technology viene sviluppata con attenzione alla composizione, alla selezione degli ingredienti e alla chiarezza delle informazioni.',
    method_panel2_p3:
      'Vogliamo costruire un rapporto di fiducia attraverso prodotti curati, comunicazione trasparente e un’identità coerente con i valori del benessere quotidiano.',
    method_panel2_li1: 'Processi chiari, pensati per garantire coerenza.',
    method_panel2_li2: 'Standard elevati, in ogni fase del percorso.',
    method_panel2_li3: 'Una visione orientata alla qualità, al valore e alla fiducia.',
    method_panel2_li4: 'Qualità percepita e sostanziale.',
    method_values_eyebrow: 'I valori del benessere',
    method_values_title: 'Energia, equilibrio,\n**serenità e qualità della vita.**',
    method_values_body:
      'Questi sono i concetti che guidano il linguaggio di 08 Natural Technology: non semplici integratori, ma prodotti pensati per accompagnare il benessere quotidiano con attenzione, qualità e rispetto.',
    method_value1_title: 'Energia',
    method_value1_body:
      'Il desiderio di sostenere la vitalità quotidiana, nei momenti in cui il corpo richiede maggiore attenzione.',
    method_value2_title: 'Equilibrio',
    method_value2_body:
      'La ricerca di formule ordinate, sensate e coerenti con le reali esigenze della persona.',
    method_value3_title: 'Serenità',
    method_value3_body:
      'La fiducia di scegliere un prodotto curato, chiaro e sviluppato senza scorciatoie comunicative.',
    method_value4_title: 'Qualità della vita',
    method_value4_body:
      'Il benessere quotidiano come obiettivo: più consapevolezza, più cura, più attenzione ai dettagli.',
    method_different_eyebrow: 'Cosa ci rende diversi',
    method_different_title: 'Alta qualità e attenzione\nsulle **formule.**',
    method_different_body:
      'Il posizionamento di 08 non nasce dal voler essere "un altro integratore", ma dal desiderio di costruire prodotti curati, con formule pensate e ingredienti selezionati evitando materie prime di scarsa qualità.',
    method_step1_title: 'Formula',
    method_step1_body:
      'Ogni prodotto nasce da una funzione precisa e da una scelta attenta degli attivi.',
    method_step2_title: 'Ingredienti',
    method_step2_body: 'La selezione degli ingredienti è uno dei punti centrali dell’identità 08.',
    method_step3_title: 'Conservazione',
    method_step3_body:
      'Il vetro farmaceutico comunica protezione, qualità e maggiore attenzione alla conservazione.',
    method_step4_title: 'Persona',
    method_step4_body:
      'Il cliente non è un numero: ogni scelta deve trasmettere rispetto, cura e fiducia.',
    method_audience_eyebrow: 'A chi ci rivolgiamo',
    method_audience_title: 'Per chi riconosce il valore della qualità.',
    method_audience_p1:
      '08 Natural Technology si rivolge a chi cerca integratori alimentari curati, chiari e sviluppati con una logica formulativa precisa.',
    method_audience_p2:
      'Ogni prodotto nasce per accompagnare il benessere quotidiano con attenzione, coerenza e rispetto verso chi sceglie il brand.',
    method_audience_li1: 'Persone che cercano prodotti affidabili e riconoscibili.',
    method_audience_li2: 'Chi desidera formule curate e facili da comprendere.',
    method_audience_li3: 'Chi vuole scegliere con maggiore consapevolezza.',
    method_madeinitaly_eyebrow: 'Made in Italy',
    method_madeinitaly_title: 'Un valore centrale, non una decorazione.',
    method_madeinitaly_p1:
      'Il Made in Italy rappresenta un elemento distintivo dell’identità di 08 Natural Technology.',
    method_madeinitaly_p2:
      'Ogni prodotto nasce in Italia, in un contesto orientato alla qualità, alla cura del dettaglio e alla coerenza del brand.',
    method_madeinitaly_p3:
      'Più che un semplice riferimento d’origine, è un valore che accompagna il progetto e ne rafforza l’affidabilità.',
    method_madeinitaly_li1: 'Identità italiana riconoscibile.',
    method_madeinitaly_li2: 'Qualità e coerenza in ogni scelta.',
    method_madeinitaly_li3: 'Un’origine che valorizza il prodotto.',
    method_madeinitaly_li4: 'Un approccio fondato su fiducia e credibilità.',
    method_lineup_eyebrow: 'Linea prodotti',
    method_lineup_title: 'Le nostre formule,\nin continuo **sviluppo.**',
    method_lineup_body:
      "08 è all'inizio del proprio percorso. Lo 0 rappresenta la nascita del progetto; l'8 rappresenta la volontà di crescere, migliorare e ampliare la linea nel tempo.",
    method_formula1_line: 'Linea Beauty',
    method_formula1_name: 'Capelli, Pelle & Unghie',
    method_formula1_body:
      'Formula dedicata con attivi selezionati ed estratti secchi naturali per capelli, pelle e unghie.',
    method_formula2_line: 'Linea Donna',
    method_formula2_name: 'MenoPausa Complex',
    method_formula2_body:
      'Supporto nutrizionale, con estratti secchi naturali, per il benessere femminile, azione giorno e notte.',
    method_formula3_line: 'Linea Microcircolo',
    method_formula3_name: 'Microcircolo Superior',
    method_formula3_body:
      'Formula pensata per il microcircolo e il drenaggio dei liquidi nelle zone periferiche.',
    method_formula4_line: 'Linea Energia',
    method_formula4_name: 'Multivitaminico & Minerali',
    method_formula4_body:
      'Formula dedicata per supportare energia, vitalità, benessere quotidiano e stress ossidativo.',
    method_formula5_line: 'In arrivo',
    method_formula5_name: 'Magnesio NP3',
    method_formula5_body: 'Neuro Performance 3: nuova formula in sviluppo.',
    method_formula_discover: 'Scopri',
    method_vision_title: 'Vision.',
    method_vision_p1:
      "**L'Eccellenza come Standard** è il principio che definisce la visione di 08 Natural Technology.",
    method_vision_p2:
      'Un modo di intendere il benessere che parte dalla qualità, dalla coerenza e dalla cura di ogni dettaglio, con l’obiettivo di costruire un’identità riconoscibile e duratura.',
    method_vision_p3:
      'Ogni formula diventa parte di un progetto più ampio: creare prodotti affidabili, curati e distintivi, pensati per accompagnare il quotidiano con serietà e valore.',
    method_vision_cta1: 'Scopri i prodotti',
    method_vision_cta2: 'Contattaci',

    // Accessibilità navbar/logo
    nav_aria_main: 'Navigazione principale',
    nav_aria_open_menu: 'Apri menu',
    nav_aria_close_menu: 'Chiudi menu',
    nav_aria_mobile_menu: 'Menu mobile',
    nav_user_fallback: 'Utente',
    logo_aria_home: '08 Natural Technology - torna alla home',

    // Accessibilità footer
    footer_aria_payments: 'Pagamenti disponibili',
    footer_payment_cod: 'Contrassegno',
    footer_payment_bank_transfer: 'Bonifico',
    footer_contact_phone_label: 'Telefono',
    footer_cookie_preferences: 'Preferenze cookie',

    // Testi generici riusabili
    common_close: 'Chiudi',
    common_back_home: 'Torna alla home',

    // Galleria prodotto
    gallery_aria_prev: 'Immagine precedente',
    gallery_aria_next: 'Immagine successiva',
    gallery_type_infographic: 'Infografica',
    gallery_type_composition: 'Composizione',
    gallery_type_back: 'Retro etichetta',
    gallery_type_label: 'Etichetta',

    // Carousel hero home
    hero_carousel_aria: 'Carousel immagini hero',
    hero_carousel_controls_aria: 'Controlli carousel hero',
    hero_carousel_goto: 'Vai alla slide {n}',

    // Popup newsletter
    newsletter_popup_aria: 'Iscrizione newsletter',

    // Banner cookie
    cookie_option_analytics_title: 'Statistiche',
    cookie_option_analytics_body:
      'Misurazione delle visite e delle interazioni, solo se verranno attivati strumenti come Analytics.',
    cookie_option_marketing_title: 'Marketing',
    cookie_option_marketing_body:
      'Misurazione campagne e pubblicità personalizzata, solo se verranno attivati strumenti come Meta Pixel o Google Ads.',
    cookie_option_newsletter_title: 'Newsletter',
    cookie_option_newsletter_body:
      'Tracciamenti collegati a newsletter, automazioni o comunicazioni commerciali, solo se presenti.',
    cookie_kicker: 'Preferenze privacy',
    cookie_title: 'Gestione cookie',
    cookie_intro:
      'Usiamo cookie tecnici necessari per far funzionare carrello, checkout, sicurezza e preferenze. Gli strumenti facoltativi per statistiche, marketing o newsletter saranno attivati solo dopo consenso.',
    cookie_read_policy: 'Leggi la Cookie Policy',
    cookie_necessary_title: 'Necessari',
    cookie_necessary_body: 'Richiesti per il funzionamento del sito. Sempre attivi.',
    cookie_always_active: 'Sempre attivi',
    cookie_reject_optional: 'Rifiuta facoltativi',
    cookie_save_preferences: 'Salva preferenze',
    cookie_reject: 'Rifiuta',
    cookie_customize: 'Personalizza',
    cookie_accept: 'Accetta',

    // Widget chat WhatsApp
    chat_aria_panel: 'Assistenza WhatsApp',
    chat_aria_close: 'Chiudi assistenza WhatsApp',
    chat_direct_support: 'Assistenza diretta',
    chat_body: 'Hai bisogno di un consiglio sui prodotti o vuoi parlare con noi per un ordine? Scrivici direttamente su WhatsApp.',
    chat_aria_open_chat: 'Apri la chat WhatsApp',
    chat_open_whatsapp: 'Apri WhatsApp',
    chat_aria_open_widget: 'Apri assistenza WhatsApp',

    // Valori nutrizionali prodotto
    ingredients_nutritional_values: 'Valori nutrizionali',
    ingredients_table_ingredient: 'Ingrediente',
    ingredients_table_per_dose: 'Per dose',
    ingredients_table_vnr: '%VNR',

    // Carrello
    cart_free_shipping_from: 'Spedizione gratuita da {amount}.',

    // Pagina prodotto (server component)
    product_all_products: 'Tutti i prodotti',
    product_made_in_italy: 'Made in Italy',
    product_kicker_template: '{line} - Formula mirata',
    product_section_usage: "Modo d'uso",
    product_section_target: 'A chi è rivolto',
    product_section_format: 'Formato e composizione',
    product_section_ingredients: 'Ingredienti',
    product_fallback_usage: 'Seguire le indicazioni riportate in etichetta.',
    product_fallback_target: 'Pensato per chi cerca un supporto nutrizionale mirato.',
    product_fallback_ingredients: 'Ingredienti non ancora specificati.',
    product_capsules_suffix: 'capsule vegetali',
    product_days_suffix: 'giorni',

    // Blog
    blog_all_articles: 'Tutti gli articoli',

    // Checkout — intestazione pagina
    checkout_secure_purchase: 'Acquisto sicuro',

    // Esiti pagina /newsletter (link nelle email di conferma/disiscrizione)
    newsletter_outcome_eyebrow: 'Newsletter',
    newsletter_outcome_confirmed_title: 'Iscrizione confermata',
    newsletter_outcome_confirmed_body:
      'Grazie. Ti abbiamo inviato una email con il tuo codice di extra sconto del 5%, da usare al checkout.',
    newsletter_outcome_confirmed_no_email_body:
      "La tua iscrizione è attiva, ma non siamo riusciti a inviarti l'email con il codice sconto. Scrivici e te lo forniamo subito.",
    newsletter_outcome_expired_title: 'Link scaduto',
    newsletter_outcome_expired_body:
      'Il link di conferma resta valido 24 ore. Iscriviti di nuovo per riceverne uno nuovo.',
    newsletter_outcome_invalid_title: 'Link non valido',
    newsletter_outcome_invalid_body:
      'Questo link di conferma non è più utilizzabile: potrebbe essere già stato usato.',
    newsletter_outcome_unsubscribed_title: 'Iscrizione annullata',
    newsletter_outcome_unsubscribed_body:
      'Non riceverai più le nostre comunicazioni. Se cambi idea puoi iscriverti di nuovo in qualsiasi momento.',
    newsletter_outcome_unsub_invalid_body:
      'Non siamo riusciti a trovare l’iscrizione collegata a questo link. Potrebbe essere già stata annullata.',

    // Form contatti — esiti invio
    contact_send_success: 'Messaggio inviato correttamente.',
    contact_send_error_generic: 'Invio non riuscito. Riprova tra poco.',

    // Coupon — errori di validazione (lib/actions/coupon.ts)
    coupon_error_empty: 'Inserisci un codice sconto',
    coupon_error_invalid: 'Codice non valido',
    coupon_error_inactive: 'Codice non attivo',
    coupon_error_expired: 'Codice scaduto',
    coupon_error_exhausted: 'Codice esaurito',
    coupon_error_not_applicable: 'Codice non applicabile al tuo account',
    coupon_error_min_order: 'Importo minimo ordine: {amount}',

    // Autenticazione — errori API (register, reset-password)
    auth_error_invalid_data: 'Dati non validi',
    auth_error_password_mismatch: 'Le password non coincidono',
    auth_error_email_taken: 'Email già registrata',
    auth_error_link_expired: 'Link scaduto o non valido',

    // Newsletter — errori API (app/api/newsletter/route.ts)
    newsletter_error_invalid_email: 'Inserisci una email valida.',
    newsletter_error_consent_required: "Devi accettare l'informativa privacy per iscriverti.",
  },
  en: {
    nav_home: 'Home',
    nav_products: 'Products & Shop',
    nav_quality: '08 Quality',
    nav_blog: 'Blog',
    nav_careers: 'Careers',
    nav_contact: 'Contact',
    nav_cart: 'Cart',
    nav_login: 'Log in',
    nav_account: 'My account',
    nav_admin: 'Admin panel',
    nav_logout: 'Log out',
    nav_register: 'Sign up',
    nav_menu: 'Menu',
    lang_selector: 'Language selector',
    lang_coming_soon: 'Coming soon',

    cart_out_of_stock: 'Out of stock',
    cart_decrease_qty: 'Decrease quantity',
    cart_increase_qty: 'Increase quantity',
    cart_added: 'Added to cart',
    cart_add: 'Add to cart',

    cart_title: 'Cart',
    cart_empty: 'Your cart is empty',
    cart_continue_shopping: 'Continue shopping',
    cart_free_shipping_earned: "You're eligible for free shipping!",
    cart_free_shipping_missing: 'Add {amount} for free shipping',
    cart_coupon_placeholder: 'Discount code',
    cart_coupon_apply: 'Apply',
    cart_coupon_applied: 'Code {code} applied',
    cart_coupon_remove: 'Remove',
    cart_subtotal: 'Subtotal',
    cart_discount: 'Discount',
    cart_shipping: 'Shipping',
    cart_shipping_free: 'Free',
    cart_total: 'Total',
    cart_checkout: 'Proceed to checkout',
    cart_checkout_note: 'International and cash-on-delivery surcharges calculated at checkout',

    footer_company: 'Company',
    footer_legal: 'Legal',
    footer_couriers: 'Couriers',
    footer_rights: 'All rights reserved',
    footer_made_in_italy: 'Made in Italy',
    footer_returns_shipping: 'Returns & shipping',
    footer_terms: 'Terms & conditions',
    footer_privacy: 'Privacy policy',
    footer_cookie_policy: 'Cookie policy',
    footer_legal_notices: 'Legal notices',

    home_hero_title: '**Excellence**\nas Standard.',
    home_hero_body:
      "08 Natural Technology was born from a dream: to create something we could be truly proud of. We didn't just want to make supplements, but bring to life products developed with passion, care and respect for the people who put their trust in us every day.",
    home_hero_cta: 'Discover our formulas',
    home_hero_alt: '08 Natural Technology',
    home_lab_alt: '08 Natural Technology laboratory',

    home_feature_1_title: 'Italian excellence',
    home_feature_1_body:
      'Italian identity, attention to detail and high standards in every choice.',
    home_feature_2_title: 'High-quality ingredients',
    home_feature_2_body: 'Carefully selected raw materials and consistent formulas.',
    home_feature_3_title: 'Research and innovation',
    home_feature_3_body: 'Modern nutritional solutions with high bioavailability.',
    home_feature_4_title: 'Wellness and real results',
    home_feature_4_body: 'Concrete solutions, designed for targeted needs.',

    home_about_eyebrow: 'About us',
    home_about_script: "Founder's Letter",
    home_about_title: 'A dream that became\na **daily promise.**',
    home_about_quote:
      "For us, wellness isn't just a word: it's energy, balance, serenity and quality of life.",
    home_about_p1:
      '**08 Natural Technology** was born from the desire to create food supplements crafted with care, with understandable formulas, selected ingredients and a recognizable Italian identity.',
    home_about_p2:
      "We didn't just want to make products, but a brand with formulas capable of conveying trust, care and respect for those who choose to take care of themselves every day.",
    home_about_signature: 'Vito Tatulli',

    home_formulas_eyebrow: 'Our formulas',
    home_formulas_title: 'Products designed\nfor **real needs.**',
    home_formulas_body:
      'From products for microcirculation and light legs, to vitamin support, hair, skin and nail beauty, and female balance during menopause: 08 Natural Technology offers nutritional solutions designed for concrete, everyday needs.',
    home_formulas_cta: 'Discover our products',

    home_blog_eyebrow: '08 Blog',
    home_blog_title: 'Insights and everyday wellness',
    home_blog_body:
      'Discover tips, ingredients and useful articles to help you navigate formulas, needs and everyday choices.',
    home_reading_time: 'min read',

    contact_eyebrow: "Let's talk",
    contact_title: 'Contact.',
    contact_where_to_find_us: 'Where to find us',
    contact_company_title: 'VIPHARMA di\n**Tatulli Vito & Co.**',
    contact_row_address_label: 'Address',
    contact_row_address_value: 'Via Don Luigi Sturzo 44/46/48 - Bitonto (BA) 70032, Italy',
    contact_row_phone_label: 'Phone',
    contact_row_whatsapp_label: 'WhatsApp',
    contact_row_email_label: 'Email',
    contact_row_social_label: 'Social',
    contact_row_social_value: 'Follow us for updates and content.',
    contact_write_us: 'Write to us',
    contact_form_title: 'Send us a\n**message.**',
    contact_field_name: 'Full name',
    contact_field_email: 'Email',
    contact_field_subject: 'Subject',
    contact_subject_products: 'Product information',
    contact_subject_collab: 'Collaborations',
    contact_subject_distribution: 'Distribution',
    contact_subject_other: 'Other',
    contact_field_message: 'Message',
    contact_send: 'Send message',
    contact_field_error: 'Please fill in your name, email and message correctly.',

    b2b_eyebrow: 'B2B',
    b2b_hero_title: 'Become an\n**08 Natural Technology** reseller.',
    b2b_hero_p1:
      'A page dedicated to pharmacies, drugstores, herbalist shops and industry professionals. Here the site collects qualified contacts and catalog requests from business partners interested in the 08 Natural Technology line.',
    b2b_hero_p2:
      "We're present in pharmacies, drugstores and herbalist shops. If you're an industry professional and want to bring 08 into your store, contact us to receive the commercial catalog and supply terms.",
    b2b_hero_cta: 'Request information',
    b2b_offer_title: 'What we offer\nto **resellers.**',
    b2b_offer_1: 'Product catalog',
    b2b_offer_2: 'PDF technical sheets',
    b2b_offer_3: 'Trade price list',
    b2b_offer_4: 'Display materials',
    b2b_offer_5: 'Social media content',
    b2b_offer_6: 'Training support',
    b2b_offer_7: 'Sales assistance',
    b2b_offer_8: 'Promotional materials',
    b2b_zone_label: 'Distribution area',
    b2b_why_eyebrow: 'Why 08',
    b2b_why_title: 'A brand that builds trust\nat the **point of sale.**',
    b2b_trust_1_label: 'Notified products',
    b2b_trust_1_body:
      'Food supplements notified to the Italian Ministry of Health where indicated.',
    b2b_trust_2_label: 'Made in Italy',
    b2b_trust_2_body: 'A project born in Bitonto, Puglia. Clear and recognizable Italian identity.',
    b2b_trust_3_label: 'Carefully crafted formulas',
    b2b_trust_3_body:
      'Selected ingredients, declared dosages, responsible and transparent communication.',

    checkout_empty: 'Your cart is empty.',
    checkout_go_to_products: 'Go to products',
    checkout_data_shipping: 'Details & shipping',
    checkout_email: 'Email',
    checkout_email_placeholder: 'customer@example.test',
    checkout_email_note: "You'll receive your order confirmation at this address.",
    checkout_already_account: 'Already have an account?',
    checkout_create_account: 'Create an account with these details',
    checkout_password: 'Password',
    checkout_password_placeholder: 'Create a secure password',
    checkout_password_note:
      "You'll be able to log in with this email and password to check your orders.",
    checkout_first_name: 'First name',
    checkout_first_name_placeholder: 'First name',
    checkout_last_name: 'Last name',
    checkout_last_name_placeholder: 'Last name',
    checkout_fiscal_doc: 'Tax document',
    checkout_doc_none: 'No document',
    checkout_doc_receipt: 'Receipt',
    checkout_doc_invoice: 'Invoice',
    checkout_fiscal_code_optional: 'Tax code (optional)',
    checkout_fiscal_code_placeholder: 'Tax code if required',
    checkout_company: 'Company name',
    checkout_company_placeholder: 'Company name',
    checkout_vat: 'Tax code or VAT number',
    checkout_vat_placeholder: 'Tax code or VAT number',
    checkout_sdi_code: 'SDI code',
    checkout_pec: 'PEC',
    checkout_sdi_or_pec_note:
      'Enter at least the SDI code or PEC (required for Italian e-invoicing).',
    checkout_address: 'Address',
    checkout_address_placeholder: 'Street and house number',
    checkout_city: 'City',
    checkout_city_placeholder: 'Delivery city',
    checkout_postal_code: 'Postal code',
    checkout_postal_code_placeholder: 'Postal code',
    checkout_province: 'Province',
    checkout_province_placeholder: 'Province code',
    checkout_phone_optional: 'Phone (optional)',
    checkout_phone: 'Phone',
    checkout_phone_prefix_label: 'Phone prefix',
    checkout_phone_placeholder: 'Phone number',
    checkout_shipping_country: 'Shipping country',
    checkout_foreign_surcharge_note: 'International shipping surcharge: +{amount}',
    checkout_billing_different: 'Billing address is different from shipping address',
    checkout_billing_address_title: 'Billing address',
    checkout_billing_first_name_placeholder: 'Billing first name',
    checkout_billing_last_name_placeholder: 'Billing last name',
    checkout_company_optional: 'Company (optional)',
    checkout_vat_optional: 'Tax code or VAT number (optional)',
    checkout_billing_city_placeholder: 'Billing city',
    checkout_province_short: 'Prov.',
    checkout_country: 'Country',
    checkout_save_next_time: 'Save these details for next time',
    checkout_shipping_notes: 'Shipping notes (optional)',
    checkout_shipping_notes_placeholder: 'E.g. intercom, floor, preferred time window',
    checkout_delivery_method: 'Delivery method',
    checkout_delivery_home: 'Home delivery',
    checkout_delivery_home_desc: 'The courier delivers to the address provided',
    checkout_delivery_pickup: 'Pickup point',
    checkout_delivery_pickup_desc: 'Choose a BRT or Poste Italiane pickup point near you',
    checkout_islands_poste_only: 'For islands only **Poste Italiane** is available as a pickup point.',
    checkout_carrier: 'Carrier',
    checkout_carrier_brt: 'BRT Fermopoint',
    checkout_carrier_poste: 'Poste Italiane',
    checkout_pickup_search_zip: 'ZIP code',
    checkout_pickup_search_zip_placeholder: 'e.g. 70032',
    checkout_pickup_search_button: 'Search',
    checkout_pickup_none_found: 'No pickup point found',
    checkout_pickup_zip_error: 'Enter a valid ZIP code (5 digits)',
    checkout_pickup_network_error: 'Network error during search',
    checkout_brt_use_location: 'Use my location',
    checkout_brt_locating: 'Searching…',
    checkout_brt_distance: '{distance} away',
    checkout_brt_type_locker: 'Locker',
    checkout_brt_type_shop: 'BRT point',
    checkout_brt_geolocation_unavailable: 'Geolocation is not available on this device',
    checkout_brt_geolocation_denied: 'Geolocation permission denied',
    checkout_brt_invalid_coordinates: 'Invalid coordinates',
    checkout_brt_service_unavailable: 'BRT service is currently unavailable',
    checkout_brt_cod_disabled: 'Cash on delivery is not available with BRT pickup points.',
    checkout_poste_search_label: 'Search pickup point by ZIP code',
    checkout_poste_zip_placeholder: 'e.g. 00164',
    checkout_poste_hours_weekday: 'Mon-Fri',
    checkout_poste_hours_saturday: 'Sat',
    checkout_poste_config_missing: 'Automatic selection requires Poste Italiane credentials to be enabled.',
    checkout_poste_service_unavailable: 'Poste Italiane service is currently unavailable',
    checkout_brt_phone_required: 'Phone number is required for pickup point delivery',
    checkout_discount_code: 'Discount code',
    checkout_discount_code_placeholder: 'Promo code',
    checkout_apply: 'Apply',
    checkout_remove: 'Remove',
    checkout_code_discount_applied: 'Code {code} — {amount} discount',
    checkout_payment_method: 'Payment method',
    checkout_pay_stripe_label: 'Card / PayPal / Google Pay / Apple Pay',
    checkout_pay_stripe_desc: 'Secure online payment with Stripe',
    checkout_pay_bank_label: 'Bank transfer',
    checkout_pay_bank_desc: "You'll receive the IBAN and payment reference after confirmation",
    checkout_pay_cod_label: 'Cash on delivery',
    checkout_pay_cod_desc: 'Pay cash to the courier (+{amount} surcharge)',
    checkout_processing: 'Processing…',
    checkout_proceed_payment: 'Proceed to payment →',
    checkout_confirm_order: 'Confirm order →',
    checkout_error_email: 'Enter a valid email address to continue.',
    checkout_error_password: 'Password must be at least 8 characters.',
    checkout_edit_address: 'Edit address',
    checkout_payment_title: 'Payment',
    checkout_order_summary: 'Order summary',
    checkout_discount_with_code: 'Discount ({code})',
    checkout_free: 'Free',

    // Newsletter
    newsletter_kicker: 'Newsletter 08',
    newsletter_title: 'Extra 5% off',
    newsletter_body: 'Subscribe for news, content and 08 Natural Technology formulas.',
    newsletter_button: 'Subscribe',
    newsletter_email_label: 'Email',
    newsletter_email_placeholder: 'name@email.com',
    newsletter_sending: 'Sending...',
    newsletter_consent_before: 'I have read the',
    newsletter_consent_link: 'privacy policy',
    newsletter_consent_after:
      'and I agree to receive marketing communications. You can unsubscribe at any time.',
    newsletter_success: 'We sent you an email: confirm your address to receive the code.',
    newsletter_error: 'We could not complete your subscription.',
    newsletter_optin_register:
      'I want to receive the newsletter with news and offers. You can unsubscribe at any time.',
    newsletter_account_title: 'Newsletter',
    newsletter_account_active: 'You are subscribed to the newsletter.',
    newsletter_account_inactive: 'You are not subscribed to the newsletter.',
    newsletter_account_subscribe: 'Subscribe me',
    newsletter_account_unsubscribe: 'Unsubscribe',
    checkout_foreign_surcharge: 'International surcharge',
    checkout_cod_surcharge: 'Cash-on-delivery surcharge',
    checkout_trust_stripe: 'Secure payment with Stripe',
    checkout_trust_bank: "You'll receive bank details by email",
    checkout_trust_cod: 'Pay cash on delivery',
    checkout_payment_error: 'Payment error',
    checkout_confirm_and_pay: 'Confirm and pay',

    success_order_received: 'Order received',
    success_thank_you: 'Thank you for your purchase.',
    success_bank_instructions_title: 'Bank transfer instructions',
    success_bank_instructions_body:
      'Complete the transfer within **5 business days**. Your order will be processed once payment is received.',
    success_bank_holder: 'Account holder',
    success_bank_iban: 'IBAN',
    success_bank_reason: 'Payment reference',
    success_bank_reason_fallback: 'Order number (see email)',
    success_cod_title: 'Payment on delivery',
    success_cod_body:
      'Payment is made directly to the courier upon delivery. Have the exact amount ready in cash. The **€5.00** cash-on-delivery surcharge is already included in the total.',
    success_stripe_body:
      "Your order has been received and confirmed. You'll receive an email with shipping details as soon as your package is on its way.",
    success_confirmation_email_note: "You'll receive a confirmation email with your order summary.",
    success_view_orders: 'View my orders',
    success_continue_shopping: 'Continue shopping',

    products_hero_title: 'Our\n**Products.**',
    products_hero_body: 'Formulas designed for real needs, with selected ingredients.',
    products_slide_kicker_suffix: 'Targeted formula',
    products_capsules: '{n} vegetable capsules',
    products_days: '{n} days',
    products_discover: 'Discover',
    products_disclaimer:
      'Food supplements are not a substitute for a varied and balanced diet and a healthy lifestyle. Keep out of reach of children. All products are notified to the Italian Ministry of Health where indicated.',

    product_regulatory_notice:
      'Food supplement notified to the Italian Ministry of Health{ms}. Do not exceed the recommended daily dose. Food supplements are not a substitute for a varied and balanced diet and a healthy lifestyle. Keep out of reach of children.',

    work_hero_title: 'Work **with us.**',
    work_hero_body:
      'We look for collaborations built on expertise, trust and real attention to project quality.',
    work_eyebrow: 'Growing with method',
    work_title: 'A young project,\nwith **clear standards.**',
    work_intro:
      '08 Natural Technology considers collaborations with people and organizations who share a serious, clear approach oriented toward everyday wellness. Every relationship is built on respect for the product, the customer, and how the brand is represented.',
    work_area_1_title: 'Sales',
    work_area_1_body:
      'Collaborations with people able to convey the value of the product with precision, method and customer care.',
    work_area_2_title: 'Consulting and outreach',
    work_area_2_body:
      'Professionals interested in contributing to clear, responsible language consistent with 08 quality.',
    work_area_3_title: 'Local partners',
    work_area_3_body:
      'Relationships with local businesses, pharmacies, drugstores, herbalist shops and wellness professionals.',
    work_cta_title: 'Tell us about yourself.',
    work_cta_body:
      'If you think you could contribute to the growth of 08 Natural Technology, send us a brief introduction: your experience, area of interest and proposed collaboration.',
    work_cta_button: 'Contact us',

    blog_eyebrow: 'Science, nutrition, wellness',
    blog_script: 'Insights',
    blog_title_1: 'Insights',
    blog_title_2: 'Blog.',
    blog_search_eyebrow: 'Search articles',
    blog_search_title: "Find the topic\nyou're **interested in.**",
    blog_search_field_label: 'Search the blog',
    blog_search_placeholder: 'E.g. diosmin, menopause, microcirculation...',
    blog_section_eyebrow: '08 Blog',
    blog_results_for: 'Results for "{query}"',
    blog_results_suggested: 'Articles suggested based on the searched topic.',
    blog_results_none: 'No articles found for this search.',
    blog_section_dossier_title: 'Product dossier',
    blog_section_dossier_body: 'Full posts dedicated to 08 Natural Technology formulas.',
    blog_section_principles_title: 'Active ingredients, ingredients and functions',
    blog_section_principles_body:
      'Separate posts dedicated to individual actives: what they are, which ingredient is used and what function they perform in the formula.',
    blog_section_quality_title: 'Ingredient quality: useful comparisons',
    blog_section_quality_body:
      'Separate posts explaining the difference between a technical ingredient and its generic alternative.',
    blog_section_other_title: 'Existing insights',
    blog_section_other_body: 'Articles kept in the blog archive.',
    blog_read_article: 'Read article',
    blog_reading_time: 'min read',

    account_title: 'My account — 08 Natural Technology',
    account_eyebrow: 'Personal area',
    account_default_title: 'My account',
    account_b2b_badge: 'B2B reseller',
    account_stat_orders: 'Orders placed',
    account_stat_total_spent: 'Total spent',
    account_my_orders: 'My orders',
    account_no_orders_title: 'No orders yet.',
    account_no_orders_body: 'Explore the catalog and find the product that suits you.',
    account_discover_products: 'Discover products',
    account_item_singular: 'item',
    account_item_plural: 'items',
    account_details: 'Details',
    account_link_profile: 'My profile',
    account_link_catalog: 'Catalog',
    account_link_support: 'Support',
    order_status_pending: 'Pending',
    order_status_paid: 'Paid',
    order_status_shipped: 'Shipped',
    order_status_delivered: 'Delivered',
    order_status_cancelled: 'Cancelled',

    profile_title: 'My profile — 08 Natural Technology',
    profile_back_to_account: 'My account',
    profile_heading: 'My profile',
    profile_saved: 'Changes saved',
    profile_personal_data: 'Personal details',
    profile_full_name: 'Full name',
    profile_phone: 'Phone',
    profile_fiscal_code: 'Tax code',
    profile_email: 'Email',
    profile_company_data: 'Company details',
    profile_company_data_hint: '(for invoicing and B2B purchases)',
    profile_company_name: 'Company name',
    profile_vat_number: 'VAT number',
    profile_sdi_code: 'SDI code',
    profile_pec: 'PEC',
    profile_save_changes: 'Save changes',
    profile_shipping_addresses: 'Shipping addresses',
    profile_new_address: '+ New',
    profile_no_saved_address: 'No saved address',
    profile_add_address: 'Add address',
    profile_default_badge: 'Default',
    profile_edit: 'Edit',
    profile_set_default: 'Set as default',
    profile_delete: 'Delete',

    // Addresses (new / edit)
    address_new_heading: 'New address',
    address_edit_heading: 'Edit address',
    address_form_label: 'Label (optional)',
    address_form_label_placeholder: 'e.g. Home, Office, Warehouse',
    address_form_default: 'Default',
    address_form_recipient: 'Recipient',
    address_form_first_name: 'First name',
    address_form_last_name: 'Last name',
    address_form_company: 'Company name (businesses)',
    address_form_vat_number: 'VAT number',
    address_form_fiscal_code: 'Tax code',
    address_form_address_section: 'Address',
    address_form_street: 'Street / Square',
    address_form_street_placeholder: 'e.g. Via Roma 1',
    address_form_postal_code: 'ZIP code',
    address_form_city: 'City',
    address_form_province: 'Prov.',
    address_form_phone: 'Phone',
    address_form_phone_placeholder: '+39 000 000 0000',
    address_form_country: 'Country',
    address_form_save_new: 'Save address',
    address_form_save_changes: 'Save changes',
    address_form_delete: 'Delete address',

    // Order detail
    order_detail_back: 'My orders',
    order_detail_label: 'Order',
    order_detail_items_ordered: 'Items ordered',
    order_detail_qty_each: 'Qty {qty} · €{price} each',
    order_detail_amount_summary: 'Amount summary',
    order_detail_subtotal: 'Subtotal',
    order_detail_discount: 'Discount',
    order_detail_total: 'Total',
    order_detail_shipping_address: 'Shipping address',
    order_detail_continue_shopping: 'Continue shopping',
    order_detail_support: 'Support',

    login_eyebrow: 'Customer area',
    login_title: 'Welcome back.',
    login_body: 'Log in to see your orders and manage your profile.',
    login_no_account: "Don't have an account?",
    login_register_link: 'Sign up',
    login_email: 'Email',
    login_password: 'Password',
    login_forgot_password: 'Forgot password?',
    login_error_unverified: 'You need to confirm your email first. Check your inbox.',
    login_error_credentials: 'Incorrect email or password.',
    login_resend_sent: 'Email sent again',
    login_resend_sending: 'Sending…',
    login_resend_verification: 'Resend confirmation email',
    login_submit: 'Log in',
    login_submitting: 'Logging in…',
    login_or: 'or',
    login_google: 'Continue with Google',
    login_verify_success: 'Email confirmed! You can now log in.',
    login_verify_expired:
      'The confirmation link has expired. Request a new one below after logging in.',
    login_verify_invalid: 'Invalid confirmation link.',
    login_reset_success: 'Password updated! Log in with your new password.',

    register_eyebrow: 'New account',
    register_title: 'Welcome.',
    register_body: 'Create your account to access orders and exclusive offers.',
    register_already_account: 'Already have an account?',
    register_login_link: 'Log in',
    register_name: 'Name',
    register_name_placeholder: 'John Smith',
    register_email_placeholder: 'john@example.com',
    register_password: 'Password',
    register_password_placeholder: 'Minimum 8 characters',
    register_confirm_password: 'Confirm password',
    register_confirm_password_placeholder: 'Repeat password',
    register_password_mismatch: 'Passwords do not match',
    register_generic_error: 'Error during registration',
    register_network_error: 'Network error. Please try again.',
    register_submit: 'Create account',
    register_submitting: 'Creating account…',
    register_terms_prefix: 'By signing up you accept our',
    register_terms_link: 'Terms and conditions of sale',
    register_and_the: 'and the',
    register_privacy_link: 'Privacy Policy',
    register_check_email_title: 'Check your email',
    register_check_email_body:
      "We've sent a link to **{email}** to confirm your address. Click the link within 24 hours to activate your account.",
    register_email_failed_title: 'Account created, email not sent',
    register_email_failed_body:
      'Your account **{email}** was created, but we could not send the confirmation email. Go to the sign-in page and use "Resend verification email".',

    forgot_eyebrow: 'Account recovery',
    forgot_title: 'Forgot your password?',
    forgot_body: "Enter your email and we'll send you a link to reset it.",
    forgot_back_to_login: 'Back to login',
    forgot_check_email_title: 'Check your email',
    forgot_check_email_body:
      "If an account exists for **{email}**, we've sent a link to reset your password. The link expires in 1 hour.",
    forgot_submit: 'Send reset link',
    forgot_submitting: 'Sending…',
    reset_title: 'Choose a new password.',
    reset_body: 'Must be at least 8 characters.',
    reset_invalid_link: 'Invalid link.',
    reset_request_new_link: 'Request a new link →',
    reset_new_password: 'New password',
    reset_confirm_new_password: 'Confirm new password',
    reset_password_mismatch: 'Passwords do not match',
    reset_generic_error: 'Error resetting password',
    reset_submit: 'Reset password',
    reset_submitting: 'Saving…',

    legal_index: 'Table of contents',
    legal_last_updated: 'Last updated',

    method_eyebrow: '08 Quality',
    method_hero_title: '**Excellence**\nas Standard.',
    method_hero_body:
      "We didn't just want to make supplements, but bring to life products developed with passion, care and respect for the people who put their trust in us every day.",
    method_hero_cta: 'Discover our formulas',
    method_promise_eyebrow: 'Our promise',
    method_promise_title: 'Quality, innovation\nand **trust.**',
    method_promise_body:
      'Behind every formula lies research, commitment and a careful choice of ingredients, because we believe quality is not a detail, but a core value.',
    method_panel1_title: 'More than a brand.',
    method_panel1_p1:
      "For us, wellness isn't just a word: it's energy, balance, serenity and quality of life. It's something precious that deserves attention and care.",
    method_panel1_p2:
      "That's why we work every day toward the same goal: offering products that combine quality, innovation and trust, always putting the person at the center of every choice.",
    method_panel1_li1: 'Formulas developed with care.',
    method_panel1_li2: 'Carefully selected ingredients.',
    method_panel1_li3: 'Clear and responsible communication.',
    method_panel1_li4: 'Respect for those who choose our brand.',
    method_panel2_title: 'The 08 promise.',
    method_panel2_p1: "For us, quality isn't just a goal: it's a method.",
    method_panel2_p2:
      'Every 08 Natural Technology formula is developed with attention to composition, ingredient selection and clarity of information.',
    method_panel2_p3:
      'We want to build a relationship of trust through carefully crafted products, transparent communication and an identity consistent with the values of everyday wellness.',
    method_panel2_li1: 'Clear processes, designed to ensure consistency.',
    method_panel2_li2: 'High standards, at every stage of the journey.',
    method_panel2_li3: 'A vision oriented toward quality, value and trust.',
    method_panel2_li4: 'Perceived and substantial quality.',
    method_values_eyebrow: 'The values of wellness',
    method_values_title: 'Energy, balance,\n**serenity and quality of life.**',
    method_values_body:
      'These are the concepts that guide the language of 08 Natural Technology: not just supplements, but products designed to support everyday wellness with care, quality and respect.',
    method_value1_title: 'Energy',
    method_value1_body:
      'The desire to support everyday vitality, in the moments when the body needs more attention.',
    method_value2_title: 'Balance',
    method_value2_body:
      'The pursuit of orderly, sensible formulas consistent with the real needs of the person.',
    method_value3_title: 'Serenity',
    method_value3_body:
      'The confidence of choosing a carefully crafted, clear product developed without communication shortcuts.',
    method_value4_title: 'Quality of life',
    method_value4_body:
      'Everyday wellness as a goal: more awareness, more care, more attention to detail.',
    method_different_eyebrow: 'What makes us different',
    method_different_title: 'High quality and attention\nto our **formulas.**',
    method_different_body:
      '08\'s positioning doesn\'t come from wanting to be "just another supplement", but from the desire to build carefully crafted products, with thoughtful formulas and selected ingredients, avoiding low-quality raw materials.',
    method_step1_title: 'Formula',
    method_step1_body:
      'Every product starts from a precise function and a careful choice of active ingredients.',
    method_step2_title: 'Ingredients',
    method_step2_body: "Ingredient selection is one of the core points of 08's identity.",
    method_step3_title: 'Preservation',
    method_step3_body:
      'Pharmaceutical glass conveys protection, quality and greater attention to preservation.',
    method_step4_title: 'The person',
    method_step4_body:
      'The customer is not a number: every choice must convey respect, care and trust.',
    method_audience_eyebrow: 'Who we serve',
    method_audience_title: 'For those who recognize the value of quality.',
    method_audience_p1:
      '08 Natural Technology is for those looking for food supplements that are carefully crafted, clear and developed with a precise formulation logic.',
    method_audience_p2:
      'Every product is designed to support everyday wellness with care, consistency and respect for those who choose the brand.',
    method_audience_li1: 'People looking for reliable, recognizable products.',
    method_audience_li2: 'Those who want carefully crafted, easy-to-understand formulas.',
    method_audience_li3: 'Those who want to choose with greater awareness.',
    method_madeinitaly_eyebrow: 'Made in Italy',
    method_madeinitaly_title: 'A core value, not a decoration.',
    method_madeinitaly_p1:
      "Made in Italy is a distinctive element of 08 Natural Technology's identity.",
    method_madeinitaly_p2:
      'Every product is born in Italy, in a context oriented toward quality, attention to detail and brand consistency.',
    method_madeinitaly_p3:
      'More than a simple reference to origin, it is a value that accompanies the project and strengthens its reliability.',
    method_madeinitaly_li1: 'Recognizable Italian identity.',
    method_madeinitaly_li2: 'Quality and consistency in every choice.',
    method_madeinitaly_li3: 'An origin that adds value to the product.',
    method_madeinitaly_li4: 'An approach built on trust and credibility.',
    method_lineup_eyebrow: 'Product line',
    method_lineup_title: 'Our formulas,\nconstantly **evolving.**',
    method_lineup_body:
      '08 is at the beginning of its journey. The 0 represents the birth of the project; the 8 represents the desire to grow, improve and expand the line over time.',
    method_formula1_line: 'Beauty Line',
    method_formula1_name: 'Hair, Skin & Nails',
    method_formula1_body:
      'Dedicated formula with selected active ingredients and natural dry extracts for hair, skin and nails.',
    method_formula2_line: "Women's Line",
    method_formula2_name: 'MenoPause Complex',
    method_formula2_body:
      'Nutritional support, with natural dry extracts, for female wellness, day and night action.',
    method_formula3_line: 'Microcirculation Line',
    method_formula3_name: 'Microcircolo Superior',
    method_formula3_body:
      'Formula designed for microcirculation and fluid drainage in peripheral areas.',
    method_formula4_line: 'Energy Line',
    method_formula4_name: 'Multivitamin & Minerals',
    method_formula4_body:
      'Dedicated formula to support energy, vitality, everyday wellness and oxidative stress.',
    method_formula5_line: 'Coming soon',
    method_formula5_name: 'Magnesio NP3',
    method_formula5_body: 'Neuro Performance 3: new formula in development.',
    method_formula_discover: 'Discover',
    method_vision_title: 'Vision.',
    method_vision_p1:
      '**Excellence as Standard** is the principle that defines the vision of 08 Natural Technology.',
    method_vision_p2:
      'A way of understanding wellness that starts from quality, consistency and attention to every detail, with the goal of building a recognizable and lasting identity.',
    method_vision_p3:
      'Every formula becomes part of a larger project: creating reliable, carefully crafted and distinctive products, designed to accompany everyday life with seriousness and value.',
    method_vision_cta1: 'Discover our products',
    method_vision_cta2: 'Contact us',

    // Navbar/logo accessibility
    nav_aria_main: 'Main navigation',
    nav_aria_open_menu: 'Open menu',
    nav_aria_close_menu: 'Close menu',
    nav_aria_mobile_menu: 'Mobile menu',
    nav_user_fallback: 'User',
    logo_aria_home: '08 Natural Technology - back to home',

    // Footer accessibility
    footer_aria_payments: 'Available payment methods',
    footer_payment_cod: 'Cash on delivery',
    footer_payment_bank_transfer: 'Bank transfer',
    footer_contact_phone_label: 'Phone',
    footer_cookie_preferences: 'Cookie preferences',

    // Reusable generic strings
    common_close: 'Close',
    common_back_home: 'Back to home',

    // Product gallery
    gallery_aria_prev: 'Previous image',
    gallery_aria_next: 'Next image',
    gallery_type_infographic: 'Infographic',
    gallery_type_composition: 'Composition',
    gallery_type_back: 'Back label',
    gallery_type_label: 'Label',

    // Home hero carousel
    hero_carousel_aria: 'Hero image carousel',
    hero_carousel_controls_aria: 'Hero carousel controls',
    hero_carousel_goto: 'Go to slide {n}',

    // Newsletter popup
    newsletter_popup_aria: 'Newsletter signup',

    // Cookie banner
    cookie_option_analytics_title: 'Statistics',
    cookie_option_analytics_body:
      'Measuring visits and interactions, only if tools like Analytics are enabled.',
    cookie_option_marketing_title: 'Marketing',
    cookie_option_marketing_body:
      'Campaign measurement and personalized advertising, only if tools like Meta Pixel or Google Ads are enabled.',
    cookie_option_newsletter_title: 'Newsletter',
    cookie_option_newsletter_body:
      'Tracking linked to newsletters, automations or commercial communications, only if present.',
    cookie_kicker: 'Privacy preferences',
    cookie_title: 'Cookie settings',
    cookie_intro:
      "We use essential technical cookies to make cart, checkout, security and preferences work. Optional tools for statistics, marketing or newsletter will only be activated after consent.",
    cookie_read_policy: 'Read the Cookie Policy',
    cookie_necessary_title: 'Necessary',
    cookie_necessary_body: 'Required for the site to work. Always active.',
    cookie_always_active: 'Always active',
    cookie_reject_optional: 'Reject optional',
    cookie_save_preferences: 'Save preferences',
    cookie_reject: 'Reject',
    cookie_customize: 'Customize',
    cookie_accept: 'Accept',

    // WhatsApp chat widget
    chat_aria_panel: 'WhatsApp support',
    chat_aria_close: 'Close WhatsApp support',
    chat_direct_support: 'Direct support',
    chat_body: 'Need advice on our products or want to talk to us about an order? Message us directly on WhatsApp.',
    chat_aria_open_chat: 'Open WhatsApp chat',
    chat_open_whatsapp: 'Open WhatsApp',
    chat_aria_open_widget: 'Open WhatsApp support',

    // Product nutritional values
    ingredients_nutritional_values: 'Nutritional values',
    ingredients_table_ingredient: 'Ingredient',
    ingredients_table_per_dose: 'Per serving',
    ingredients_table_vnr: '%NRV',

    // Cart
    cart_free_shipping_from: 'Free shipping from {amount}.',

    // Product page (server component)
    product_all_products: 'All products',
    product_made_in_italy: 'Made in Italy',
    product_kicker_template: '{line} - Targeted formula',
    product_section_usage: 'How to use',
    product_section_target: "Who it's for",
    product_section_format: 'Format & composition',
    product_section_ingredients: 'Ingredients',
    product_fallback_usage: 'Follow the instructions on the label.',
    product_fallback_target: 'Designed for those looking for targeted nutritional support.',
    product_fallback_ingredients: 'Ingredients not yet specified.',
    product_capsules_suffix: 'vegetable capsules',
    product_days_suffix: 'days',

    // Blog
    blog_all_articles: 'All articles',

    // Checkout — page header
    checkout_secure_purchase: 'Secure purchase',

    // /newsletter outcome page (confirmation/unsubscribe email links)
    newsletter_outcome_eyebrow: 'Newsletter',
    newsletter_outcome_confirmed_title: 'Subscription confirmed',
    newsletter_outcome_confirmed_body:
      "Thank you. We've sent you an email with your extra 5% discount code, to use at checkout.",
    newsletter_outcome_confirmed_no_email_body:
      "Your subscription is active, but we couldn't send you the email with the discount code. Contact us and we'll provide it right away.",
    newsletter_outcome_expired_title: 'Link expired',
    newsletter_outcome_expired_body:
      'The confirmation link stays valid for 24 hours. Sign up again to receive a new one.',
    newsletter_outcome_invalid_title: 'Invalid link',
    newsletter_outcome_invalid_body:
      'This confirmation link can no longer be used: it may have already been used.',
    newsletter_outcome_unsubscribed_title: 'Subscription cancelled',
    newsletter_outcome_unsubscribed_body:
      'You will no longer receive our communications. If you change your mind, you can subscribe again at any time.',
    newsletter_outcome_unsub_invalid_body:
      "We couldn't find the subscription linked to this link. It may have already been cancelled.",

    // Contact form — submit outcomes
    contact_send_success: 'Message sent successfully.',
    contact_send_error_generic: 'Unable to send. Please try again.',

    // Coupon — validation errors (lib/actions/coupon.ts)
    coupon_error_empty: 'Enter a discount code',
    coupon_error_invalid: 'Invalid code',
    coupon_error_inactive: 'Inactive code',
    coupon_error_expired: 'Expired code',
    coupon_error_exhausted: 'Code exhausted',
    coupon_error_not_applicable: 'Code not applicable to your account',
    coupon_error_min_order: 'Minimum order amount: {amount}',

    // Auth — API errors (register, reset-password)
    auth_error_invalid_data: 'Invalid data',
    auth_error_password_mismatch: "Passwords don't match",
    auth_error_email_taken: 'Email already registered',
    auth_error_link_expired: 'Expired or invalid link',

    // Newsletter — API errors (app/api/newsletter/route.ts)
    newsletter_error_invalid_email: 'Enter a valid email address.',
    newsletter_error_consent_required: 'You must accept the privacy policy to subscribe.',
  },
} as const satisfies Record<string, Record<string, string>>

export type DictionaryKey = keyof typeof dictionary.it

/** Traduce una chiave, sostituendo i segnaposto {nome} con i valori indicati. */
export function translate(
  locale: Locale,
  key: DictionaryKey,
  vars?: Record<string, string>
): string {
  const selected = dictionary[locale as keyof typeof dictionary] ?? dictionary.it
  const text = (selected[key] ?? dictionary.it[key]) as string
  if (!vars) return text
  return Object.entries(vars).reduce((acc, [name, value]) => acc.replace(`{${name}}`, value), text)
}
