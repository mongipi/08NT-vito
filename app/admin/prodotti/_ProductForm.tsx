...
interface Props {
  ...
  product?: Product & {
    ...
    productImages: Pick<ProductImage, 'key'>[]
...
function Field({
  ...
  rows?: number
  step?: string
  /** Nota esplicativa mostrata sotto al campo. */
  hint?: string
}) {
  return (
    <div>
      ...
      {hint && (
        <p
          style={{ fontSize: '0.75rem', color: '#9ca3af', margin: '0.25rem 0 0', lineHeight: 1.5 }}
        >
          {hint}
        </p>
      )}
  ...
  if (product) {
    for (const img of product.productImages) {
      ...
      if (path) images[img.key] = `/api/product-images/${product.id}/${path}`
        ...
        images[img.key] = `/api/product-images/${product.id}/${img.key}`
    }
  }
    ...
    <>
        ...
        <div className="product-form-grid">
          ...
          <div style={s.stack(16)}>
            <div style={s.cardPad}>
              ...
              <div style={s.stack(14)}>
                ...
                <div style={s.grid2}>
                  <div>
                    <label style={s.label}>
                      Linea<span style={{ color: '#ef4444', marginLeft: 2 }}>*</span>
                    </label>
                    <select
                      name="lineId"
                      required
                  ...
                  <Field
                    ...
                    name="order"
                    type="number"
                    defaultValue={product?.order ?? 99}
                    hint="Posizione negli elenchi (vetrina prodotti, footer, questa tabella). Numero più basso = più in alto. 99 è il valore predefinito e manda il prodotto in fondo."
                  />
                </div>
              </div>
            </div>
            ...
            <div style={s.cardPad}>
              ...
              <div style={s.stack(14)}>
                <Field
                  label="Descrizione formato"
                  name="format"
                  defaultValue={product?.format}
                  placeholder="es. 60 capsule vegetali"
                />
                <div style={s.grid2}>
                  <Field
                    label="Capsule"
                    name="capsules"
                    type="number"
                    defaultValue={product?.capsules}
                  />
                  <Field
                    label="Giorni trattamento"
                    name="days"
                    type="number"
                    defaultValue={product?.days}
                  />
                  <Field
                    label="Dosaggio"
                    name="dosage"
                    defaultValue={product?.dosage}
                    placeholder="es. 2 capsule/die"
                  />
                  <Field
                    label="Notifica Min. Salute"
                    name="notificationMs"
                    defaultValue={product?.notificationMs}
                  />
                </div>
              </div>
            </div>
            ...
            <div style={s.cardPad}>
              ...
              >
                PNG, JPG o WebP. Clicca uno slot per caricare. Le immagini esistenti restano
                invariate se non ne selezioni una nuova.
              </p>
            </div>
            ...
            <div style={s.cardPad}>
              ...
              <Field
                ...
                rows={4}
                placeholder="es. Agnocasto, Vitamina B6, Magnesio, ..."
              />
            </div>
            ...
            <div style={s.cardPad}>
              ...
              <p style={{ fontSize: '0.75rem', color: '#9ca3af', margin: '-0.375rem 0 0.875rem' }}>
                Mostrata sul sito quando il cliente seleziona EN. Se lasci un campo vuoto, resta
                visibile l&apos;italiano.
              </p>
              <div style={s.stack(14)}>
                ...
                <Field
                  ...
                  defaultValue={product?.longDescriptionEn}
                  rows={5}
                />
                <Field
                  label="Uso consigliato (EN)"
                  name="usageEn"
                ...
                <Field
                  label="Target (EN)"
                  name="targetEn"
                  defaultValue={product?.targetEn}
                  rows={2}
                />
                <Field
                  label="Descrizione formato (EN)"
                  name="formatEn"
                  defaultValue={product?.formatEn}
                />
                <Field
                  label="Elenco ingredienti (EN)"
                  name="ingredientsTextEn"
                  defaultValue={product?.ingredientsTextEn}
                  rows={4}
                />
              </div>
            </div>
          ...
          <div style={s.stack(16)}>
            <div style={s.cardPad}>
              ...
              <div style={s.stack(12)}>
                ...
                />
                <Field
                  label="Prezzo originale barrato (€)"
                  name="comparePrice"
            ...
            <ConfirmSaveButton
              ...
              message={
                product
                  ? 'Vuoi salvare le modifiche a questo prodotto?'
                  : 'Vuoi creare questo nuovo prodotto?'
              }
              style={s.btnPrimary}
            />
          </div>
      ...
      {product && deleteAction && (
        <div style={{ marginTop: '1.5rem' }}>
          ...
          >
            <p style={{ ...s.cardTitle, color: '#dc2626', marginBottom: '0.375rem' }}>
              Zona pericolosa
            </p>
            <p
              style={{
      ...
      <style>{`
        ...
        /* A colonna singola la barra laterale (prezzo, scorte, visibilita')
           finirebbe dopo tutto il resto: la si porta in cima, dove serve. */
        .product-form-grid > :last-child {
          order: -1;
        }
