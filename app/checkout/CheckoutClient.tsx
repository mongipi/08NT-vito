...
import type { DocType } from '@/types/checkout'
...
export function CheckoutClient({
  ...
  const form = useCheckoutForm(pricing, prefill)
  const {
    session,
    cart,
    step,
  ...
  const PAY_METHODS: { value: PayMethod; label: string; desc: string }[] = [
    ...
    {
      value: 'contrassegno',
      label: t('checkout_pay_cod_label'),
      desc: t('checkout_pay_cod_desc', { amount: `€${codSurcharge.toFixed(2)}` }),
    },
  ]

  const availablePayMethods = PAY_METHODS.filter(
    (opt) => !(isBrtPickup && opt.value === 'contrassegno')
  )
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 24 }} className="checkout-grid">
      ...
      <div>
        {step === 'address' && (
          <div
            style={{ background: 'white', border: '1px solid var(--border)', padding: '1.75rem' }}
          >
            <h2
              style={{
                fontSize: '0.5625rem',
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              {!session && (
                <div>
                  ...
                  />
                  <p
                    style={{ fontSize: '0.6875rem', color: 'var(--ink-4)', marginTop: '0.375rem' }}
                  >
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                {(['firstName', 'lastName'] as const).map((key) => (
                  <div key={key}>
                    <label style={labelStyle}>
                      {key === 'firstName' ? t('checkout_first_name') : t('checkout_last_name')}
                      <span style={{ color: '#ef4444', marginLeft: 2 }}>*</span>
                    </label>
                    <input
                      type="text"
                      ...
                      placeholder={
                        key === 'firstName'
                          ? t('checkout_first_name_placeholder')
                          : t('checkout_last_name_placeholder')
                      }
                    />
                  </div>
                ))}
              ...
              <div style={{ paddingTop: '0.25rem' }}>
                ...
                {docType === 'scontrino' && (
                  <div style={{ marginTop: '0.75rem' }}>
                    ...
                    <input
                      type="text"
                      value={address.fiscalCode}
                      onChange={(e) => setAddress((a) => ({ ...a, fiscalCode: e.target.value }))}
                      style={inputStyle}
                      placeholder={t('checkout_fiscal_code_placeholder')}
                    />
                  </div>
                )}
                  ...
                  >
                    <div>
                      ...
                      <input
                        ...
                        style={inputStyle}
                        placeholder={t('checkout_company_placeholder')}
                      />
                    </div>
                    <div>
                      <label style={labelStyle}>
                      ...
                      <input
                        ...
                        onChange={(e) => setAddress((a) => ({ ...a, vatNumber: e.target.value }))}
                        style={inputStyle}
                        placeholder={t('checkout_vat_placeholder')}
                      />
                    </div>
                    <div
                    ...
                    >
                      <div>
                        ...
                        <input
                          type="text"
                          value={address.sdiCode}
                          onChange={(e) => setAddress((a) => ({ ...a, sdiCode: e.target.value }))}
                          style={inputStyle}
                          placeholder={t('checkout_sdi_code')}
                          maxLength={7}
                        />
                      </div>
                      <div>
                        ...
                        <input
                          type="email"
                          value={address.pec}
                          onChange={(e) => setAddress((a) => ({ ...a, pec: e.target.value }))}
                          style={inputStyle}
                          placeholder="pec@azienda.test"
                        />
                      </div>
                    </div>
                    <p style={{ fontSize: '0.6875rem', color: 'var(--ink-4)', margin: 0 }}>
                      {t('checkout_sdi_or_pec_note')}
                    </p>
                  </div>
                )}
              </div>
              ...
              <div>
                ...
                {isEstero && (
                  <p style={{ fontSize: '0.6875rem', color: '#b45309', marginTop: '0.375rem' }}>
                    {t('checkout_foreign_surcharge_note', {
                      amount: `€${foreignSurcharge.toFixed(2)}`,
                    })}
                  </p>
                )}
              </div>
                ...
                {billingDifferent && (
                  ...
                  >
                    <p style={{ ...labelStyle, margin: 0 }}>
                      {t('checkout_billing_address_title')}
                    </p>
                    <div
                      style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}
                    >
                      {(['firstName', 'lastName'] as const).map((k) => (
                        <div key={k}>
                          <label style={labelStyle}>
                            {k === 'firstName' ? t('checkout_first_name') : t('checkout_last_name')}
                            <span style={{ color: '#ef4444', marginLeft: 2 }}>*</span>
                          </label>
                          <input
                            type="text"
                            ...
                            placeholder={
                              k === 'firstName'
                                ? t('checkout_billing_first_name_placeholder')
                                : t('checkout_billing_last_name_placeholder')
                            }
                          />
                        </div>
                      ))}
                    ...
                    <div>
                      ...
                      <input
                        type="text"
                        value={billing.vatNumber || billing.fiscalCode}
                    ...
                    >
                      ...
                      <div>
                        ...
                        <input
                          ...
                          style={inputStyle}
                          placeholder={t('checkout_billing_city_placeholder')}
                        />
                      </div>
                      <div>
                  ...
                  [
                    {
                      ...
                      desc: t('checkout_delivery_home_desc'),
                    },
                    {
                      value: 'pickup',
                      label: t('checkout_delivery_pickup'),
                      desc: t('checkout_delivery_pickup_desc'),
                    },
                  ] as { value: 'home' | 'pickup'; label: string; desc: string }[]
                ).map((opt) => (
                  <label
                    key={opt.value}
                    style={{
              ...
              {cart.coupon && (
                ...
                >
                  <span style={{ fontSize: '0.75rem', color: '#16a34a' }}>
                    ✓{' '}
                    {t('checkout_code_discount_applied', {
                      code: cart.coupon.code,
                      amount: formatPrice(cart.discountAmount),
                    })}
                  </span>
                  <button
            ...
            >
              ...
              {isBrtPickup && (
                <p style={{ fontSize: '0.75rem', color: 'var(--ink-4)', margin: '0 0 0.75rem' }}>
            ...
            <button
              ...
              style={{
                ...
                color: 'white',
                border: 'none',
                cursor: canProceed && !loading && !pending ? 'pointer' : 'not-allowed',
                fontSize: '0.6875rem',
                fontWeight: 600,
                letterSpacing: '0.14em',
                textTransform: 'uppercase',
          ...
          >
            ...
            <Elements
              ...
              options={{
                ...
                appearance: {
                  theme: 'stripe',
                  variables: { colorPrimary: '#1a4a2e', borderRadius: '0px' },
                },
              }}
            >
              <PaymentForm />
      ...
      >
        ...
        >
          {cart.items.map((item) => (
            ...
            >
              ...
              <span style={{ fontWeight: 600, color: 'var(--forest)', flexShrink: 0 }}>
          ...
          {baseShipping === 0 ? (
            ...
            <>
              ...
              {totals.foreignSurcharge > 0 && (
                ...
                >
                  <span>{t('checkout_foreign_surcharge')}</span>
                  <span>+{formatPrice(totals.foreignSurcharge)}</span>
                </div>
              )}
            </>
          )}
        ...
        >
          ...
          {payMethod === 'stripe'
            ? t('checkout_trust_stripe')
            : payMethod === 'bonifico'
              ? t('checkout_trust_bank')
              : t('checkout_trust_cod')}
        </div>
      </div>
